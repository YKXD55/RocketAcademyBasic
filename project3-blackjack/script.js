...
var issueCards = function () {
  ...
  if (setGameMode == `Player Draw Card`) {
    ...
    playerTempValue = playerCardValue;
    // console.log(`lowest card value before draw`);
    // console.log(playerLowestCardValue);

    // To include ace can be either one or 11 for player
    for (var c = 0; c < numOfCardsDrew + 2; c++) {
      if (playerCards[c].special == `aceCard`) {
        //playerLowestCardValue is meant for situation where player draw 2 aces later on.
        playerAceExist++;
      }
      // To begin calculate after third card is drawn.
      if (c >= 2) {
        ...
        if (playerCardValue > 21 && playerAceExist >= 1) {
          playerCardValue = playerLowestCardValue;
        }
      }
    }
  ...
  if (setGameMode == `Player Decision`) {
    ...
    for (var d = 0; d < numOfCardsDrew + 2; d++) {
      if (playerCards[d].special == `aceCard`) {
        //playerLowestCardValue is meant for situation where player draw 2 aces later on.
        playerAceExist++;
      }
      // To begin calculate after third card is drawn.
...
var main = function (input) {
  ...
  if (setGameMode == `Deal Cards`) {
    ...
    if (playerAutoWin == 1 && comAutoWin == 1) {
      myOutputValue = `What a coincidence, both of you drew Blackjack together. <br>
      ...
      myOutputValue = `You drew Blackjack. You Win!! <br>
      You have drew ${playerCards[0].name} of ${playerCards[0].suit} and ${playerCards[1].name} of ${playerCards[1].suit}. <br> Wanna play another round??`;
    } else if (comAutoWin == 1) {
      myOutputValue = `Com drew Blackjack. You Lose!! <br>
      You have drew ${playerCards[0].name} of ${playerCards[0].suit} and ${playerCards[1].name} of ${playerCards[1].suit}. <br> Wanna play another round?? `;
    } else {
...
Your current card value is ${playerCardValue}.<br> Do you want to Hit or Stand?<br>Enter "Hit" or "Stand"`;
  ...
  //The user's cards are analysed for winning or losing conditions.
    ...
    outputStatement += `. Your total card value is ${playerCardValue}.<br> Com have drew ${comCards[0].name} of ${comCards[0].suit}, ${comCards[1].name} of ${comCards[1].suit}`;
    ...
    if (playerCardValue > 21 && comCardValue <= 21) {
      outputStatement += `. Com card value is ${comCardValue}. <br> You burst and you lost.<br> Click on submit to restart the game.`;
    } else if (playerCardValue > 21 && comCardValue > 21) {
      outputStatement += `. Com card value is ${comCardValue}. <br> Both of you burst and it is a draw.<br> Click on submit to restart the game.`;
    } else if (playerCardValue == comCardValue) {
      outputStatement += `. Com card value is ${comCardValue}. <br> It is a tie.<br> Click on submit to restart the game.`;
    } else if (playerCardValue <= 21 && comCardValue > 21) {
      outputStatement += `. Com card value is ${comCardValue}. <br> You won, com have burst its card.<br> Click on submit to restart the game.`;
    } else if (playerCardValue > comCardValue) {
      outputStatement += `. Com card value is ${comCardValue}. <br> You won.<br> Click on submit to restart the game.`;
    } else if (playerCardValue < comCardValue) {
      outputStatement += `. Com card value is ${comCardValue}. <br> You Lost.<br> Click on submit to restart the game.`;
    }
